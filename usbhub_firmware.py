...
def upgrade_circuitpython(drive, uf2_version, uf2_file):
    ...
    with open(code_file, "w") as handle:
        ...
        handle.write(
            "this_version = '.'.join([str(v) for v in sys.implementation.version])\n"
        )
...
def check_for_usbhub(drive):
    ...
    if not os.path.exists(boot_file):
        print(
            "Specified drive '{}' does not appear to be a CircuitPython device".format(
                drive
            )
        )
        sys.exit(0)
    ...
    if (
        boot_file_string[1].strip()
        != "Capable Robot Programmable USB Hub with samd51g19"
    ):
        print(
            "Specified drive '{}' is not a Capable Robot Programmable USB Hub".format(
                drive
            )
        )
...
def circuitpython(drive, version):
    ...
    if version is None:
        ...
        if len(uf2s) == 0:
            versions = [
                file_to_version(f) for f in sorted(glob.glob("./assets/adafruit*.uf2"))
            ]
...
@click.option(
    "--force", is_flag=True, help="Force file updates (ignores file modification times)"
)
@click.argument("drive")
def firmware(main, force, drive):
    ...
    for file in glob.glob("{}/lib/*.py".format(root_dir)) + glob.glob(
        "{}/lib/**/*.py".format(root_dir)
    ):
